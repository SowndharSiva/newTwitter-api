...
const checkOwnTweet = async (request, response, next) => {
  const { user_id } = request;
  let { tweetId } = request.params;
  console.log(tweetId);
  console.log(user_id);
  const tweet_Id = parseInt(tweetId.slice(1));
  const getQuery = `SELECT tweet_id FROM tweet WHERE user_id=${user_id};`;
  const getDetails = await db.all(getQuery);
  console.log(tweet_Id);
  let isCheckTweet = false;
  const checkTweet = await getDetails.map((eachObj) => {
    console.log(parseInt(eachObj.tweet_id));
    if (eachObj.tweet_id === tweet_Id) {
      isCheckTweet = true;
    }
    console.log(isCheckTweet);
  });
  if (isCheckTweet) {
    request.details = { user_id: user_id, tweet_Id: tweet_Id };
    next();
    console.log("Have tweet");
  } else {
    response.status(401);
...
const mutualUser = async (request, response, next) => {
  ...
  const allCheckQuery = `SELECT * FROM follower INNER JOIN tweet
  ON tweet.user_id=follower.following_user_id
  WHERE follower.follower_user_id=${user_id};`;
  const allDetails = await db.all(allCheckQuery);
  console.log(allDetails);
  console.log(gettweet);
  let isTweetPresent = false;
  const checkUser = await gettweet.map((eachObj) => {
    if (eachObj.tweet_id === tweet_Id) {
      isTweetPresent = true;
    }
  });
...
app.get("/user/tweets/", authenticateUser, async (request, response) => {
  const { user_id } = request;
  const getQuery = `SELECT tweet.tweet AS tweet,COUNT(DISTINCT like.like_id) AS likes,COUNT(DISTINCT reply.reply_id) AS replies ,tweet.date_time AS dateTime 
